...
type ReportTable = { title: string; columns: string[]; rows: Record<string, unknown>[] };
...
export default function ReportIndex({ type, tabs, filters, options, report }: Props) {
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
                <PageHeader
                    ...
                    action={<Button asChild><a href={`/admin/reports/${type}/export?${query}`}><Download /> Export CSV</a></Button>}
                />

                <div className="flex flex-wrap gap-2">
                    {tabs.map((tab) => (
                        <Button key={tab} asChild variant={tab === type ? 'secondary' : 'outline'} size="sm">
                            <Link href={`/admin/reports/${tab}`}>{tab}</Link>
                        </Button>
                    ))}
                </div>

                <form onSubmit={submit} className="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-3 xl:grid-cols-6">
                    ...
                    <select value={data.category_id} onChange={(event) => setData('category_id', event.target.value)} className="border-input rounded-md border bg-transparent px-3 py-2 text-sm">
                ...
                {report.tables.map((table) => (
                    <TableShell key={table.title} title={table.title} description={`${table.rows.length} rows`}>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead><tr className="border-b text-left text-muted-foreground">{table.columns.map((column) => <th key={column} className="pb-3 pr-4 font-medium">{column.replaceAll('_', ' ')}</th>)}</tr></thead>
                                <tbody className="divide-y">{table.rows.map((row, index) => <tr key={index}>{table.columns.map((column) => <td key={column} className="py-3 pr-4">{formatCell(column, row[column])}</td>)}</tr>)}</tbody>
                            </table>
                        </div>
                    </TableShell>
                ))}
...
function formatCell(column: string, value: unknown) {
    ...
    if (['grand_total', 'revenue', 'total_spending', 'total_discount'].includes(column)) {
