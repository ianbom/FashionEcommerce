...
function ActionButton({
    icon: Icon, label, href, external = false,
}: { icon: IconComponent; label: string; href?: string | null; external?: boolean }) {
    const base =
    ...
    if (!href) {
        return (
            <button type="button" disabled className={`${base} cursor-not-allowed opacity-40`}>
...
function InfoLine({ icon: Icon, label, value }: { icon: IconComponent; label: string; value: ReactNode }) {
    return (
        <div className="flex items-start gap-3 py-2.5 text-sm">
            <Icon className="mt-0.5 shrink-0 text-[#9b8777]" size={15} strokeWidth={1.65} />
            <span className="w-28 shrink-0 text-xs text-[#8b7b6e]">{label}</span>
            <span className="min-w-0 break-words text-sm font-medium text-[#3f3025]">{value || '-'}</span>
        </div>
    );
}

function SectionCard({ title, children, noPad = false }: { title: string; children: ReactNode; noPad?: boolean }) {
    return (
        <div className="overflow-hidden rounded-2xl border border-[#EAE8E3] bg-white shadow-sm">
            <div className="border-b border-[#f0ebe4] px-5 py-4 sm:px-6">
                <h2 className="font-serif text-lg text-[#2d2119] sm:text-xl">{title}</h2>
            </div>
...
function MetaChip({ label, children }: { label: string; children: ReactNode }) {
    return (
        <div className="flex flex-col gap-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-[#9a8575]">{label}</span>
            <div className="text-sm font-semibold text-[#3d3027]">{children}</div>
        </div>
    );
}
...
function buildProgress(order: Order) {
    const paidAt =
        ...
        order.payment_logs.find((log) => log.transaction_status === 'settlement')?.processed_at ??
        null;
    ...
    return [
        { icon: ClipboardList, label: 'Order Placed', time: formatDateTime(order.created_at),                          complete: true,             active: false },
        { icon: Check,        label: 'Payment',      time: formatDateTime(paidAt),                                    complete: currentRank >= 1, active: currentRank === 1 },
        { icon: ShieldCheck,  label: 'Processing',   time: currentRank >= 2 ? labelStatus(order.order_status) : '-', complete: currentRank >= 2, active: currentRank === 2 },
        { icon: Package,      label: 'Packed',       time: currentRank >= 3 ? labelStatus(order.order_status) : '-', complete: currentRank >= 3, active: currentRank === 3 },
...
export default function DetailOrder({ order }: Props) {
    ...
    const courier = [order.shipment?.courier_company, order.shipment?.courier_type || order.shipment?.courier_service_name]
        .filter(Boolean)
        .join(' ');
    const address = order.address;
    const paymentMethod =
        [order.payment?.payment_provider, order.payment?.payment_method].filter(Boolean).join(' / ') || '-';
                    ...
                    <div className="overflow-hidden rounded-2xl border border-[#EAE8E3] bg-white shadow-sm">
                        <div className="p-5 sm:p-6">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9a8575]">Order Number</p>
                                    <h2 className="mt-1 font-serif text-2xl leading-tight text-[#2d2119] sm:text-3xl">
                                        {order.order_number}
                                    </h2>
                                    <p className="mt-1 text-xs text-[#7f6d60]">
                                        {order.created_date ?? '-'} &bull; {order.created_time ?? '-'}
                                    </p>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    <StatusPill tone={statusTone(order.payment_status)}>
                                        {labelStatus(order.payment_status)}
                                    </StatusPill>
                                    <StatusPill tone={statusTone(order.order_status)}>
                                        {labelStatus(order.order_status)}
                                    </StatusPill>
                                </div>
                            </div>
                            <div className="mt-5 grid grid-cols-2 gap-4 border-t border-[#f0ebe4] pt-5 sm:grid-cols-3">
                                <MetaChip label="Payment Method">{paymentMethod}</MetaChip>
                                <MetaChip label="Est. Arrival">{order.shipment?.estimated_delivery ?? '-'}</MetaChip>
                                <MetaChip label="Tracking No.">{order.shipment?.waybill_id ?? '-'}</MetaChip>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3 border-t border-[#f0ebe4] p-4 sm:grid-cols-4 sm:p-5">
                            <ActionButton
                                href={order.shipment?.waybill_id ? `https://cekresi.com/?noresi=${order.shipment.waybill_id}` : null}
                                external
                                icon={Truck}
                                label="Track Order"
                            />
                            <ActionButton href="/list" icon={RefreshCcw} label="Buy Again" />
                            <ActionButton href={order.shipment?.label_url ?? null} external icon={Download} label="Shipping Label" />
                            <ActionButton href="/notifications" icon={Headphones} label="Support" />
                        </div>
                    </div>
                        ...
                        <div className="hide-scrollbar overflow-x-auto pb-1">
                            <div className="relative grid min-w-[520px] grid-cols-6">
                                ...
                                {progressSteps.map((step) => {
                                    ...
                                    return (
                                        <div key={step.label} className="relative flex flex-col items-center px-1 text-center">
                                            <div
                                            ...
                                            <p className="mt-3 text-[11px] font-bold leading-tight text-[#4b3a2d]">{step.label}</p>
                                            <p className="mt-0.5 text-[10px] font-medium leading-tight text-[#a08d80]">{step.time}</p>
                                        </div>
                                    );
                                })}
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-[11px] font-semibold text-[#9a8575]">
                                                {formatDateTime(tracking.happened_at)}
                                            </p>
                    ...
                    <SectionCard title="Ordered Items" noPad={true}>
                        ...
                        <div className="hidden sm:block">
                            <div className="hide-scrollbar overflow-x-auto">
                                <table className="w-full min-w-[580px] border-collapse text-left">
                                    <thead>
                                        <tr className="border-b border-[#f0ebe4] bg-[#faf6f2] text-[11px] uppercase tracking-wide text-[#9a8575]">
                                            <th className="px-5 py-3 font-semibold sm:px-6">Item</th>
                                            <th className="px-4 py-3 font-semibold">Details</th>
                                            <th className="px-4 py-3 font-semibold">Unit Price</th>
                                            <th className="px-4 py-3 text-center font-semibold">Qty</th>
                                            <th className="px-5 py-3 text-right font-semibold sm:px-6">Subtotal</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-[#f0ebe4]">
                                        {order.items.map((item) => {
                                            ...
                                            return (
                                                <tr key={item.id} className="bg-white transition-colors hover:bg-[#fdfaf7]">
                                                    <td className="px-5 py-4 sm:px-6">
                                                        <div className="flex items-center gap-3">
                                                            <Link href={productUrl} className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-[#f4ebe4]">
                                                                <img src={item.product_image_url ?? FALLBACK_IMAGE} alt={item.product_name} className="h-full w-full object-cover object-top" />
                                                            </Link>
                                                            <div className="min-w-0">
                                                                <Link href={productUrl} className="line-clamp-2 text-[13px] font-semibold text-[#3d3027] transition hover:text-[#8f684b]">
                                                                    {item.product_name}
                                                                </Link>
                                                                <p className="mt-0.5 text-[11px] text-[#9a8575]">SKU: {item.variant_sku ?? item.product_sku ?? '-'}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-4">
                                                        <div className="space-y-1 text-[12px] text-[#6d5c50]">
                                                            <p>Color: <span className="font-semibold text-[#3d3027]">{item.color_name ?? '-'}</span></p>
                                                            <p>Size: <span className="font-semibold text-[#3d3027]">{item.size ?? '-'}</span></p>
                                                            <p>Weight: <span className="font-semibold text-[#3d3027]">{item.weight ? `${item.weight} gr` : '-'}</span></p>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-4 text-[13px] font-semibold text-[#3d3027]">{formatPrice(item.price)}</td>
                                                    ...
                                                    <td className="px-5 py-4 text-right text-[13px] font-semibold text-[#3d3027] sm:px-6">{formatPrice(item.subtotal)}</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>
                        ...
                        <div className="space-y-3 p-4 sm:hidden">
                            {order.items.map((item) => {
                                const productUrl = item.product_slug ? productShow.url({ product: item.product_slug }) : '#';
                                return (
                                    <div key={item.id} className="flex gap-3 rounded-xl border border-[#f0ebe4] bg-white p-3">
                                        <Link href={productUrl} className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[#f4ebe4]">
                                            <img src={item.product_image_url ?? FALLBACK_IMAGE} alt={item.product_name} className="h-full w-full object-cover object-top" />
                                        </Link>
                                        <div className="min-w-0 flex-1">
                                            <Link href={productUrl} className="line-clamp-2 text-[13px] font-semibold text-[#3d3027] hover:text-[#8f684b]">
                                                {item.product_name}
                                            </Link>
                                            <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-[#9a8575]">
                                                {item.color_name && <span>Color: <strong className="text-[#4a392c]">{item.color_name}</strong></span>}
                                                {item.size && <span>Size: <strong className="text-[#4a392c]">{item.size}</strong></span>}
                                                {item.weight && <span>Weight: <strong className="text-[#4a392c]">{item.weight} gr</strong></span>}
                                            </div>
                                            <p className="mt-0.5 text-[11px] text-[#9a8575]">SKU: {item.variant_sku ?? item.product_sku ?? '-'}</p>
                                            <div className="mt-2.5 flex items-center justify-between">
                                                <span className="text-xs text-[#9a8575]">Qty: <strong className="text-[#4a392c]">{item.quantity}</strong></span>
                                                <span className="text-[13px] font-bold text-[#3d3027]">{formatPrice(item.subtotal)}</span>
                                            </div>
                                        </div>
                                    </div>
                                );
                        ...
                        <SectionCard title="Payment Info">
                            <div className="divide-y divide-[#f5ede6]">
                                ...
                                <div className="flex items-center gap-3 py-2.5">
                                    <WalletCards className="shrink-0 text-[#9b8777]" size={15} strokeWidth={1.65} />
                                    <span className="w-28 shrink-0 text-xs text-[#8b7b6e]">Status</span>
                ...
                <aside className="space-y-5 xl:sticky xl:top-6 xl:self-start">
                    ...
                    <SectionCard title="Order Summary">
                        <div className="space-y-2.5">
                            ...
                            {order.discount_amount > 0 && (
                                <SummaryRow label="Discount" value={`− ${formatPrice(order.discount_amount)}`} danger />
                            )}
                            {order.voucher_code && (
                        ...
                        <div className="mt-5 rounded-xl bg-[#faf6f1] px-4 py-4">
                            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#9a8575]">Total Payment</p>
                            <p className="mt-1 font-serif text-2xl font-medium leading-none text-[#221914] sm:text-3xl">
                    ...
                    <SectionCard title="Invoice & Docs">
                        <div className="mb-4 flex items-center justify-between gap-2">
                            <span className="text-xs text-[#8b7b6e]">Invoice No.</span>
                            <span className="truncate text-[13px] font-bold text-[#3d3027]">{invoiceNumber}</span>
                        </div>
                        <div className="space-y-2.5">
                            <ActionButton href={order.shipment?.label_url ?? null} external icon={Download} label="Shipping Label" />
                            <ActionButton href={order.payment?.midtrans_redirect_url ?? null} external icon={FileText} label="Payment Page" />
                        </div>
                    </SectionCard>
