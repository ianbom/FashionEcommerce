...
{
    private array $paymentStatuses = ['pending', 'paid', 'expired', 'failed', 'cancelled'];

    private array $orderStatuses = ['pending_payment', 'paid', 'processing', 'ready_to_ship', 'shipped', 'delivered', 'completed', 'cancelled', 'expired'];

    private array $shippingStatuses = ['not_created', 'confirmed', 'allocated', 'picked', 'in_transit', 'delivered', 'cancelled', 'problem'];
    ...
    public function updateStatus(OrderStatusRequest $request, Order $order): RedirectResponse
    {
        ...
        if ($target === 'cancelled' && $order->payment_status === 'paid') {
            throw ValidationException::withMessages([
                'status' => 'Order paid tidak bisa dibatalkan dari dashboard tanpa flow khusus.',
            ]);
        }
        ...
        if (! $allowed) {
            throw ValidationException::withMessages([
                'status' => 'Perubahan status tidak sesuai flow order.',
            ]);
        }
        ...
        $this->notifyOrderStatus($order->fresh(), $target);

        return back()->with('success', 'Order status berhasil diperbarui.');
    }

    public function updateNotes(OrderNoteRequest $request, Order $order): RedirectResponse
    {
    ...
    private function notifyOrderStatus(Order $order, string $status): void
    {
        ...
        Notification::query()->create([
            ...
            'message' => "Order {$order->order_number} sekarang berstatus {$status}.",
            'type' => 'order',
