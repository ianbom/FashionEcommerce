...
class PaymentController extends Controller
{
    ...
    public function sync(Payment $payment): RedirectResponse
    {
    ...
    private function markOrderPaid(Order $order, Payment $payment): void
    {
        ...
        $this->notifyPayment($order, 'Payment received', "Payment untuk order {$order->order_number} berhasil diterima.");
    }
    ...
    private function markOrderFailed(Order $order, string $status): void
    {
        ...
        $this->notifyPayment($order, 'Payment updated', "Payment untuk order {$order->order_number} berstatus {$status}.");
    }
    ...
    private function notifyPayment(Order $order, string $title, string $message): void
    {
        if (! $order->user_id) {
            return;
        }
